module GenerateQR exposing (Model, Msg, emptyModel, update, view)
...
getCreateError err =
    case err of
        MinDataLength ->
            "You must provide data to encode"

        MinSize ->
            "QR code is too small (needs to be at least 10)"

        MaxSize format maxSize ->
            "For format " ++ formatToPathString format ++ " maximum size is " ++ String.fromInt maxSize

        MaxDataLength ->
            "The data can be at most 900 chars of length"

        MarginLow ->
            "Please provide a positive value for margin"

        MarginHigh ->
            "Margin can not be above 50"

        QZoneLow ->
            "Please provide a positive value for qZone"

        QZoneHigh ->
            "QZone can not be above 100"

        LowContrast currentVal ->
            "Contrast ratio of the current given colors is " ++ String.fromFloat currentVal ++ ". Please make sure it is at least 3.0 for better looking"
...
emptyQr =
    CreatedQRCode "Input Data" 200 False L Color.black Color.white 0 0 SVG
...
emptyModel : Model
emptyModel =
    Model emptyQr ColorPicker.empty ColorPicker.empty Tab.initialState
...
update msg model =
    ...
    case msg of
        ColorPicker cMsg ->
            let
                ( m, color ) =
                    ColorPicker.update cMsg model.qrCode.color model.colorPicker
            in
            { model | qrCode = { qr | color = Maybe.withDefault qr.color color }, colorPicker = m }

        BgColorPicker cMsg ->
            let
                ( m, color ) =
                    ColorPicker.update cMsg model.qrCode.bgColor model.bgColorPicker
            in
            { model | qrCode = { qr | bgColor = Maybe.withDefault qr.bgColor color }, bgColorPicker = m }

        TabMsg tState ->
            { model | tabState = tState }
            ...
            let
                newQr =
                    case other of
                        ...
                        ChangeSize size ->
                            { qr | size = parseInt size }

                        SetCharset val ->
                            { qr | isoCharset = val }

                        SetErrorCorrection ecc ->
                            { qr | ecc = fromStringToCorrection ecc }

                        ChangeMargin margin ->
                            { qr | margin = parseInt margin }

                        ChangeQZone qZone ->
                            { qr | qZone = parseInt qZone }

                        SetFormat format ->
                            { qr | format = pathToFormatString format }

                        _ ->
                            qr

                -- The cases are handled in above case statements
            in
            { model | qrCode = newQr }
...
getQRUrl qr =
    let
        contrast =
            Color.Accessibility.contrastRatio qr.color qr.bgColor
    in
    if String.length qr.data < 1 then
        Err MinDataLength

    else if String.length qr.data > 900 then
        Err MaxDataLength

    else if qr.size < 10 then
        Err MinSize

    else if qr.size > 1000 && List.member qr.format [ PNG, GIF, JPG ] then
        Err (MaxSize qr.format 1000)

    else if qr.size > 1000000 then
        Err (MaxSize qr.format 1000000)

    else if contrast < 3.0 then
        Err (LowContrast contrast)
        -- https://www.w3.org/TR/WCAG20-TECHS/G17.html

    else if qr.margin < 0 then
        Err MarginLow

    else if qr.margin > 50 then
        Err MarginHigh

    else if qr.qZone < 0 then
        Err QZoneLow

    else if qr.qZone > 100 then
        Err QZoneHigh

    else
        Ok (buildQRCodeUrl qr)


convertColor : Color.Color -> String
convertColor color =
    String.dropLeft 1 (Color.Convert.colorToHex color)


buildQRCodeUrl : CreatedQRCode -> String
buildQRCodeUrl qr =
    Builder.crossOrigin "https://api.qrserver.com/"
        [ "v1", "create-qr-code" ]
        [ Builder.string "data" qr.data
        , Builder.string "size" (String.fromInt qr.size ++ "x" ++ String.fromInt qr.size)
        , Builder.string "charset-source"
            (if qr.isoCharset then
                "ISO-8859-1"
...
renderResult res =
    case res of
        ...
        Err err ->
            div [] [ text ("Error: " ++ getCreateError err) ]
...
viewBasicsForm qr =
    Form.form []
        ...
        , Form.group []
            ...
            , Select.select [ Select.id "format", Select.onChange SetFormat ]
                (List.map
                    (\val ->
                        let
                            pathString =
                                formatToPathString val
                        in
                        Select.item [ A.id pathString, A.value pathString ] [ text (formatToText val) ]
                    )
                    [ PNG, GIF, JPG, SVG ]
                )
...
viewCard model =
    Grid.container []
        [ Grid.row []
            [ Grid.col [ Col.offsetSm2, Col.sm3 ]
                [ case getQRUrl model.qrCode of
                    Ok url ->
                        img [ A.src url, A.style "margin-left" "auto", A.style "margin-right" "auto", A.style "background" "url('loading.gif') no-repeat" ] []

                    Err err ->
                        Alert.simpleDanger [] [ text (getCreateError err) ]
                ]
            , Grid.col [ Col.sm5 ]
